...
class Money(BaseModel):
    ...
    indexed_by: Optional[str] = Field(None)
...
class Franchise(BaseModel):
    ...
    type_franchise: Optional[str] = Field(None)
...
class ConditionApplication(BaseModel):
    ...
    max_unoccupied_days: Optional[int] = Field(
        None,
...
class Exclusion(BaseModel):
    ...
    full_text: str = Field(
        ...
        examples=["Sont exclus les vols commis sans effraction ni violence sur la personne.", "Dommages résultant d'un défaut d'entretien notoire."]
    )
...
class Garantie(BaseModel):
    ...
    name: str = Field(
        ...
        examples=["Incendie et événements assimilés", "Vol et vandalisme", "Responsabilité Civile Vie Privée"]
    )
    ...
    plafond_garantie: Optional[Money] = edge(
        label="A_PLAFOND",
...
class Section(BaseModel):
    ...
    sous_sections: List["Section"] = edge(
        label="A_SOUS_SECTION",
        default_factory=list,
        description="Sous-sections imbriquées."
    )
